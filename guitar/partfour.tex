...
learn a new type of chord\ldots{} what is known as a \term{power chord}. You'll also
...
Before we begin, it is very important to understand the way the \term{musical
...
is \term{E}. Now, let's figure out the other note names on the sixth string.
...
chord contains the \term{root note} - the root of a C power chord is \term{C} - and
another note called the \term{fifth}. For this reason, power chords are often
referred to as \term{fifth chords} (written C5 or E5, etc).
...
the sixth string, it would be referred to as an \term{A power chord}, since the note
on the fifth fret of the sixth string is A. If the chord were played on the
eighth fret, it would be a \term{C power chord}. This is why it is important to know
...
\begin{itemize}
\item Power chords are often referred to as a \term{fifth} or \term{5} chord. If you see
...
\item Many guitarists prefer to use all downstrokes when strumming power
      chords. This results in a more \q{chunky} sound. This is not a rule, only an
      observation
\end{itemize}
...
loud \term{down, down up, up down} (or \term{1, 2 and, and 4}) as you're playing the
pattern.
...
\item Don't strum too hard, as this causes strings to rattle, and produces an
      undesirable sound
\item Don't strum too softly, as this produces a \q{wimpy} sound. Your pick
